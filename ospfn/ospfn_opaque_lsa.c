...
void 
ndnx_opaque_lsa_print (struct ndnx_opaque_lsa  *col)
{
	...
	switch( opaque_type)
	{
		case NAME_OPAQUE_LSA:
			ndnx_name_opaque_lsa_print((struct ndnx_opaque_lsa *)col);
			break;
		case ADJ_OPAQUE_LSA:
			break;
 	}
...
void 
update_opaque_lsa(struct ndnx_opaque_lsa *col)
{
	struct lsa_header *lsah = &col->lsah;
	int opaque_type=ntohl(lsah->id.s_addr)>>24;
	...
	{
		case NAME_OPAQUE_LSA:
			update_name_opaque_lsa((struct ndnx_opaque_lsa *) col);
			break;
		case ADJ_OPAQUE_LSA:
			break;  
	}
...
void 
delete_opaque_lsa(struct ndnx_opaque_lsa *col)
{
	...
	{
		case NAME_OPAQUE_LSA:
			delete_name_opaque_lsa((struct ndnx_opaque_lsa *) col);
			break;
		case ADJ_OPAQUE_LSA:
			break;
	}
