...
init(void)
{
	...
	ospfn->local_scope_template = ndn_charbuf_create();
	//unsigned char ndndid_storage[32] = {0};
	//ospfn->ndndid=ndndid_storage;	
	ospfn->ndndid=calloc(33,sizeof(char));
...
usage(char *progname)
{
	...
	Announces name prefix LSAs and modifies ndnd FIB.\n\n\
...
/**
*
* process ndnname commands and call for name OLS injection
*
*/
void 
process_command_ndnname(char *command)
{
	if(command==NULL)
	{
		writeLogg(ospfn->logFile,__FILE__,__FUNCTION__,__LINE__,"Wrong Command Format [ ndnname /name/prefix opaque_id]\n");
		return;
	}
	...
	/* procesing of ndn name commands here */
...
/**
*
* process ndnnametype command and set nametype uri/ndnb
*
*/

void
process_command_ndnnametype(char *command)
{
	if(command==NULL)
	{
		writeLogg(ospfn->logFile,__FILE__,__FUNCTION__,__LINE__," Wrong Command Format ( ndnnametype uri/ndnb\n");
		return;
	}
	char *rem;
	const char *sep=" \t\n";
	...
	if(!strcmp(strToLower(nametype),"ndnb"))
	{
...
process_conf_command(char *command,int isLogOnlyProcessing)
{
	...
	if(!strcmp(cmd_type,"ndnname") )
	{
		if (isLogOnlyProcessing == 0)
			process_command_ndnname(remainder);
	}
	else if(!strcmp(cmd_type,"ndnnametype") )
	{
		if (isLogOnlyProcessing == 0)
			process_command_ndnnametype(remainder);
	} 
        ...
        struct lsa_header *lsa)
{
	...
	{
		update_opaque_lsa((struct ndnx_opaque_lsa *) lsa);
		ndnx_opaque_lsa_print((struct ndnx_opaque_lsa *) lsa);
    	}
}
        ...
        struct lsa_header *lsa)
{
    	...
    	if(lsa->type == 9 || lsa->type == 10 || lsa->type == 11)
	{
		delete_opaque_lsa((struct ndnx_opaque_lsa *) lsa);
    	}
}
...
{	
	...
	readConfigFile(config_file,1);
    	...
    	ndn_handle = ndn_create();
    	res = ndn_connect(ndn_handle, NULL); 
    	if (res < 0) {
		ndn_perror(ndn_handle, "Cannot connect to ndnd.");
		exit(1);
    	}

	init_data(ospfn->local_scope_template);
	ospfn->ndndid_size = get_ndndid(ndn_handle, ospfn->local_scope_template, ospfn->ndndid);
	if ((int)ospfn->ndndid_size != 32 ){
		fprintf(stderr, "Incorrect size for ndnd id in response\n");
		fprintf(stderr, "Exiting from ospfn\n");
		exit (1);
	}else{
		printf("ndnd_id Size:%d  ndnd_id: %s \n",(int)ospfn->ndndid_size,ospfn->ndndid);
	}
